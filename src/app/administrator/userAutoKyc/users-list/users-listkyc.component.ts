...
@Component({
  selector: 'app-users-listkyc',
  templateUrl: './users-listkyc.component.html',
  styleUrls: ['./users-listkyc.component.scss']
})
export class UsersListkycComponent implements OnInit, AfterViewInit, AfterViewChecked {
  form: FormGroup;
  ...
  filterTable(): void {
    this.router.navigate(['dashboard/users-listkyc'], { queryParams: this.form.value});
  ...
  populateTable(obj): void {
    ...
    this.usersService
      .getKycUserTblDetails(data)
      .subscribe(
        (result) => {
          // let res =result.getUsersTblDetails
          // let filter = res;
