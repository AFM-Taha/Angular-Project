...
@Component({
  ...
  templateUrl: './futures-subacc-view.component.html',
  styleUrls: ['./futures-subacc-view.component.scss']
})
export class FuturesSubaccViewComponent implements OnInit {
  _id: any;
  form: FormGroup;
  userDetails: any = {};
  isLoading = true;
  ...
  p2pPaymentdetails = new MatTableDataSource<any>();
    ...
    if (tabChangeEvent.index > 0) {
      switch (tabChangeEvent.index) {
        ...
        case 5:
          ...
          break;
        case 6:
          this.getP2PPaymentList({ 'id': this.userDetails._id });
          break;
        default:
  ...
  ExportToCsv(exportkeyparam) {
    ...
    } else if (exportkeyparam == 'P2Ppayment') {
      this.filename = 'Payment Details';
      this.p2pPaymentdetails.data.forEach(element => {
        data.push({
          'Created Date': this.datepipe.transform(element.createdDate, 'MMM dd, y, HH:mm:ss'),
          'Payment Method': element.paymenttype,
          'Details': element.paymenttype == "Bank" ? element.accountNo : element.upiId,
          'Status': element.status == 0 ? 'Pending' : element.status == 1 ? 'Verified' : element.status == 2 ? 'Rejected' : element.status == 4 ? 'Deleted' : element.status == 5 ? 'Disabled' : '',
        })
      });
    }
  ...
  setbankSuspendReason(value: string) {
    this.bankSuspendReason = value;
  }
  ...
  setKYCStatus(value: string, type: string) {
  ...
  getP2PPaymentList(obj) {
    ...
    this.p2pService
      .getP2PPaymentTblDetails({ data })
      .subscribe(
        (result) => {
          if (result.getp2ppaymentDetails.length > 0) {
            this.p2pPaymentdetails.data = result.getp2ppaymentDetails;
            this.p2pPaymentlength = result.total;
            this.isLoading = false;
          } else {
            this.p2pPaymentdetails.data = [];
            this.p2pPaymentlength = result.total;
            this.isLoading = false;
          }
        },
  ...
  suspendPayment(): void {
    ...
    let data = {
      _id: this._id,
      bankSuspend: this.userDetails.bankSuspend,
      bankSuspendReason: this.bankSuspendReason
    }
    this.usersService
      .updateSuspend(data)
      .subscribe(
  ...
  addBank(): void {
    const dialogRef = this.dialog.open(P2PPaymentFormComponent, {
      width: '500px',
      height: '630px',
      data: { editData: '', userId: this.userDetails._id }
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res.data.status) {
        this.populateValue(this.form.value);
      }
    })
  }
  editPayment(_id: string): void {
    const i = this.p2pPaymentdetails.data.findIndex(function (object: any) {
      return object._id === _id;
    });
    let a = this.p2pPaymentdetails.data[i]
    const dialogRef = this.dialog.open(P2PPaymentFormComponent, {
      width: '500px',
      height: '630px',
      data: { editData: a, userId: this.userDetails._id }
    });
    dialogRef.afterClosed().subscribe(res => {
      if (res.data.status) {
        this.populateValue(this.form.value);
  ...
  changepage(event, objs) {
    if (event.pageSize != this.limit) {
      ...
      else if (objs == 'p2ppayment') {
        this.getP2PPaymentList(obj1)
      }
  ...
  deleteData(target: string, data: string) {
    var reqData = {
      _id: this._id,
      'target': target,
      'passData': data,
    }
    this.usersService.userDetailsRemoved({ reqData }).subscribe((result) => {
      if (result.status) {
        this.notify.showSuccess(result.message);
        this.populateValue(this.form);
        this.isLoading = false;
  ...
  editData(target: string, data: string) {
    var reqData = {
      _id: this._id,
      'target': target,
      'email': data,
    }
    this.usersService.userEmailUpdation({ reqData }).subscribe((result) => {
      if (result.status) {
        this.notify.showSuccess(result.message);
        this.populateValue(this.form);
        this.isLoading = false;
