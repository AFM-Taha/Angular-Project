...
import { DerivativesPairs } from 'src/app/_models/derivatives-pairs';
...
@Component({
  selector: 'app-derivatives-list',
  templateUrl: './derivatives-list.component.html',
  styleUrls: ['./derivatives-list.component.scss']
})
export class DerivativesPairsListComponent implements OnInit, AfterViewInit, AfterViewChecked {
  form:FormGroup;
  ...
  resultsLength = 0;
  pairsTbl = new MatTableDataSource<DerivativesPairs>();
  ...
  filterTable(){
    this.router.navigate(['dashboard/derivatives-pairs-list'], { queryParams: this.form.value});
  ...
  populateTable(obj): void {
    ...
    this.currencyService
      .getDerivativesPairsDetailsfilter(data)
      .subscribe(
        (result) => {
  ...
  addPairs(): void {
    const dialogRef = this.dialog.open(DerivativesPairsFormComponent, {
      width: '500px',
      height: '630px',
