...
@Component({
  selector: 'app-buyorder-list',
templateUrl: './buyorder-list.component.html',
styleUrls: ['./buyorder-list.component.scss']
})
export class BuyorderListComponent implements OnInit, AfterViewInit, AfterViewChecked {
  form: FormGroup;
  ...
  populateTable(obj): void {
    ...
    this.transactionsService
      .getBuycryptoTblDetails(data)
      .subscribe(
        (result:any) => {
          console.log(result.data);
           this.transactionsTbl.data = result.data;
           this.isLoading = false;
           this.length=result.data.total;
           this.transactionsTbl.paginator = this.paginator;
           this.transactionsTbl.sort = this.sort;
        },
        (err) => {
  ...
  viewUsers(_id: string): void {
    this.router.navigate(['dashboard', 'buyorder-view', _id]);
  }
  nextPage() {
