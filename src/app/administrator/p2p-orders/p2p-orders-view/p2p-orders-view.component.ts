...
@Component({
  selector: 'app-p2p-orders-view',
  templateUrl: './p2p-orders-view.component.html',
  styleUrls: ['./p2p-orders-view.component.scss']
})
export class P2POrdersViewComponent implements OnInit {
  
  _id: any;
  status: string = '';
  p2pordersDetails : any = {};
  ...
  populateValue() {
    this.P2PService
      .getp2pOrdersDetails({_id:this._id})
      .subscribe(
        (result) => {
          if(result.status) {
            this.p2pordersDetails = result.data;
            console.log("p2pordersDetails:",this.p2pordersDetails)
  ...
  cancelOrder() {
    if (confirm('Are you sure cancel order?')) {
      let data = { transactiondet: this.p2pordersDetails,reason: this.userReason}
        this.P2PService
        .p2pcancelOrder({data})
        .subscribe(
          (result) => {
